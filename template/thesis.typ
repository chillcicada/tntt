...
// 文稿设置，默认应用 LaTeX/i-figured 引用兼容模式（引用标签时可添加 `fig:` 等前缀），不影响原有标签的使用
// 设置 `use-latexref` 为 false 后可关闭 LaTeX/i-figured 引用兼容模式，将在未来调整为默认启用
// 默认设置 `<->` 标签为不编号的行间公式标签，设置 `unnumbered-label` 为 "" 或 none 以禁用此行为
#show: it => meta(it)
// 上面的写法等价于如下写法，但由于技术限制，该写法会导致 meta 的类型提示损坏，因而此处显式使用函数调用
// #show: meta
...
- #strike[不支持多参考文献源，]不支持多参考文献实例。理论上可以通过 #link("https://typst.app/universe/package/alexandria", underline[alexandria]) 包解决，由于 Typst 官方的相关开发正在推进，因而并未采用。考虑单参考文献实例对论文编辑影响并不严重，目前在对附录部分部分的参考文献处理较为粗暴，可以理解为没有实现双向链接的参考文献样式，这样做主要是考虑到成就页部分的参考文献一般不会被正文引用，因而如果在附录中有链接参考的需求，您可能需要手动用 `cite` 函数来引用对应的参考文献或手动管理编号链接。
- ……
